...
public class MyClient {
	...
	public MyClient(String host, int port) {
		...
		try {		
			...
			text = readFile("dick.txt");
			Job<TextChunk, Integer, Integer> newJob = new MyJob(text, searchedString, new MyTask());
			RemoteFuture<Integer> rf =  server.doJob(newJob);
			int oldRresult=-1;
			while(!rf.isFinished()){
				
				if (rf.get()!=oldRresult) {
